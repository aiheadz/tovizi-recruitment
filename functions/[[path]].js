...
const SINGLE_ADMIN_PASSWORD = 'mesike';
...
async function validSession(request, env) {
  ...
  try{const payload=new TextDecoder().decode(fromB64url(parts[0])),pieces=payload.split(':'),userId=Number(pieces[0]),issued=Number(pieces[2]); if(userId!==1||!Number.isFinite(issued)||Date.now()-issued>SESSION_TTL*1000)return null; const key=await crypto.subtle.importKey('raw',new TextEncoder().encode(env.ADMIN_SESSION_SECRET),{name:'HMAC',hash:'SHA-256'},false,['verify']); if(!await crypto.subtle.verify('HMAC',key,fromB64url(parts[1]),new TextEncoder().encode(payload)))return null; return {id:1,name:'Admin',email:''};}catch{return null;}
}
