...
function showLogin(msg=""){
  app.innerHTML =
    ...
    '<div class="brand">Tóvizi <span>Recruitment</span></div>' +
    '<h1>Admin belépés</h1>' +
    '<p class="muted">A weboldal üzenetei és személyre szabott linkjei innen kezelhetők.</p>' +
    '<div class="error">'+msg+'</div>' +
    '<input id="pw" type="password" placeholder="Admin jelszó">' +
    '<button class="btn" style="width:100%;margin-top:12px" onclick="login()">Belépés</button>' +
    '</div>';
...
function render(s){
  app.innerHTML =
    ...
    '<div>' +
    '<div class="brand">Tóvizi <span>Recruitment</span></div>' +
    '<div class="muted">Admin felület</div>' +
    '</div>' +
    '<button class="btn light" onclick="logout()">Kilépés</button>' +
    '</div>' +

    '<div class="grid">' +
    '<div class="stat">Összes üzenet<b>'+s.stats.total+'</b></div>' +
    '<div class="stat">Olvasatlan<b>'+s.stats.unread+'</b></div>' +
    '<div class="stat">Kezelés alatt<b>'+s.stats.progress+'</b></div>' +
    '<div class="stat">Aktív link<b>'+s.stats.links+'</b></div>' +
    '</div>' +

    '<div class="tabs">' +
    '<button class="tab active" onclick="tab(this,\\'messages\\')">📩 Üzenetek</button>' +
    '<button class="tab" onclick="tab(this,\\'links\\')">🔗 Személyre szabott linkek</button>' +
    '</div>' +
    ...
    '<div class="muted">Az űrlapról érkező megkeresések kezelése.</div>' +
    '</div>' +
    '<select id="filter" onchange="loadMessages()">' +
    '<option value="all">Minden státusz</option>' +
    '<option value="unread">Olvasatlan</option>' +
    '<option value="progress">Kezelés alatt</option>' +
    ...
    '<div class="top">' +
    '<div>' +
    '<h2>Személyre szabott linkek</h2>' +
    '<div class="muted">6 számjegyű, egyedi URL-ek.</div>' +
    '</div>' +
    '<button class="btn" onclick="newLink()">+ Új személy</button>' +
    '</div>' +
    '<div id="linkList"></div>' +
    '</section>' +
...
function tab(btn,id){
  ...
  document.querySelectorAll("#messages,#links").forEach(function(x){
...
async function loadMessages(){
  ...
  document.getElementById("messageList").innerHTML =
    ...
    d.items.map(function(x){
      ...
      '<td><b>'+esc(x.name)+'</b><br><span class="muted">'+esc(x.company||"")+'</span></td>' +
      '<td>'+esc(x.contact)+'</td>' +
      '<td>'+esc(x.message||"").slice(0,180)+'</td>' +
      '<td><span class="pill '+x.status+'">'+statusHu(x.status)+'</span></td>' +
      '<td><div class="actions">' +
      '<button class="btn light small" onclick="viewMsg('+x.id+')">Megnyitás</button>' +
      '<button class="btn light small" onclick="setMsg('+x.id+',\\'progress\\')">Kezelés</button>' +
      '<button class="btn light small" onclick="setMsg('+x.id+',\\'closed\\')">Lezárás</button>' +
      '<button class="btn red small" onclick="delMsg('+x.id+')">Törlés</button>' +
      '</div></td>' +
      '</tr>';
    }).join("") +
...
async function loadLinks(){
  ...
  document.getElementById("linkList").innerHTML =
    ...
    d.items.map(function(x){
      return '<tr>' +
      '<td><b>'+esc(x.full_name)+'</b><br><span class="muted">'+esc(x.first_name)+'</span></td>' +
      '<td><code>'+location.origin+'/'+x.code+'</code></td>' +
      '<td>'+x.visit_count+'</td>' +
      '<td>'+esc(x.last_visit||"—")+'</td>' +
      '<td><span class="pill">'+(x.active?"Aktív":"Deaktivált")+'</span></td>' +
      '<td><div class="actions">' +
      '<button class="btn light small" onclick="copyLink(\\''+x.code+'\\')">Link másolása</button>' +
      '<button class="btn light small" onclick="toggleLink('+x.id+','+(x.active?0:1)+')">'+(x.active?"Deaktiválás":"Aktiválás")+'</button>' +
      '<button class="btn red small" onclick="delLink('+x.id+')">Törlés</button>' +
      '</div></td>' +
      '</tr>';
    }).join("") +
...
function statusHu(s){
  if(s==="unread") return "Olvasatlan";
  if(s==="progress") return "Kezelés alatt";
  return "Lezárt";
}
...
async function viewMsg(id){
  ...
  document.getElementById("modal").innerHTML =
    ...
    '<div class="top">' +
    '<h2>'+esc(x.name)+'</h2>' +
    '<button class="btn light" onclick="closeModal()">Bezárás</button>' +
    '</div>' +
    '<p><b>Cég:</b> '+esc(x.company||"—")+'</p>' +
    '<p><b>Elérhetőség:</b> '+esc(x.contact)+'</p>' +
    '<p><b>Érkezett:</b> '+esc(x.created_at)+'</p>' +
    '<p><b>Forrás:</b> '+esc(x.source_code ? location.origin+"/"+x.source_code : "Normál oldal")+'</p>' +
    '<hr>' +
...
function newLink(){
  ...
  document.getElementById("modal").innerHTML =
    '<div class="modalbox">' +
    '<div class="top">' +
    '<h2>Új személyre szabott link</h2>' +
    '<button class="btn light" onclick="closeModal()">Bezárás</button>' +
    '</div>' +
    ...
    '<input id="fullName" placeholder="Gipsz Jakab">' +
    '</div>' +

    '<div class="full">' +
    '<label>Megszólítás / keresztnév</label>' +
    '<input id="firstName" placeholder="Levente">' +
    '</div>' +
    '</div>' +

    '<button class="btn" style="margin-top:14px" onclick="createLink()">Link generálása</button>' +
...
async function createLink(){
  ...
  if(!full_name || !first_name){
    alert("A teljes név és a keresztnév is szükséges.");
    return;
  }
  ...
  box.innerHTML =
    '<div class="generated-title">Elkészült a személyre szabott link</div>' +
    '<div class="generated-row">' +
    '<input id="generatedUrl" value="'+link+'" readonly>' +
    '<button class="btn" onclick="copyLink(\\''+d.code+'\\')">Link másolása</button>' +
    '</div>';
}
...
async function delLink(id){
  if(!confirm("Biztosan törlöd ezt a személyre szabott linket?")) return;
...
async function apiHandler(request, env) {
  ...
  if(msgMatch){
    ...
    if(request.method === "PATCH"){
      ...
      if(
        !["unread","progress","closed"].includes(status)
      ){
        return json({
...
async function personalizedPage(request,env,code){
  ...
  const block =
    ...
    '<span>'+first+',</span>' +
    '<strong>nézzük meg, hogyan segíthetünk.</strong>' +
    '</div>';
  ...
  const css =
    ...
    '.personal-greeting{margin-bottom:18px}' +
    '}' +
    '</style>';
      ...
      (function(){
        ...
        f.addEventListener("submit",async function(e){
          ...
          const payload={
            ...
            message:ta?.value||"",
            source_code:window.__TR_CODE__||null
          };
          ...
          try{
            ...
            alert("Az üzenetet most nem sikerült elküldeni. Kérjük, próbáld újra.");
          }finally{
