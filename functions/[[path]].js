...
async function validSession(request, env) {
  ...
  if (!ts || !sig || Date.now() - Number(ts) > 1000 * 60 * 60 * 24 * 7) {
    return false;
  }
...
async function loadMessages(){
  ...
  document.getElementById('messageList').innerHTML=d.items.length
    ? '<table><thead><tr><th>Dátum</th><th>Érdeklődő</th><th>Elérhetőség</th><th>Üzenet</th><th>Státusz</th><th>Műveletek</th></tr></thead><tbody>'
      ...
      +'">'
      +statusHu(x.status)
      +'</span></td><td><div class="actions"><button class="btn light small" onclick="viewMsg('
      +x.id
      +')">Megnyitás</button><button class="btn light small" onclick="setMsg('
      +x.id
      +',\\'progress\\')">Kezelés</button><button class="btn light small" onclick="setMsg('
      +x.id
      +',\\'closed\\')">Lezárás</button><button class="btn red small" onclick="delMsg('
      +x.id
      +')">Törlés</button></div></td></tr>').join('')
      +'</tbody></table>'
...
async function loadLinks(){
  ...
  document.getElementById('linkList').innerHTML=d.items.length
    ? '<table><thead><tr><th>Név</th><th>Link</th><th>Megnyitás</th><th>Utolsó megnyitás</th><th>Státusz</th><th>Műveletek</th></tr></thead><tbody>'
      +d.items.map(x=>'<tr><td><b>'
      +esc(x.full_name)
      +'</b><br><span class="muted">'
      +esc(x.first_name)
      +'</span></td><td><code>'
      +location.origin+'/'+x.code
      +'</code></td><td>'
      +x.visit_count
      +'</td><td>'
      +esc(x.last_visit||'—')
      +'</td><td><span class="pill">'
      +(x.active?'Aktív':'Deaktivált')
      +'</span></td><td><div class="actions"><button class="btn light small" onclick="copyLink(\\''
      +x.code
      +'\\')">Link másolása</button><button class="btn light small" onclick="toggleLink('
      +x.id
      +','
      +(x.active?0:1)
      +')">'
      +(x.active?'Deaktiválás':'Aktiválás')
      +'</button><button class="btn red small" onclick="delLink('
      +x.id
      +')">Törlés</button></div></td></tr>').join('')
      +'</tbody></table>'
...
async function viewMsg(id){
  ...
  document.getElementById('modal').innerHTML='<div class="modalbox"><div class="top"><h2>'
    +esc(x.name)
    +'</h2><button class="btn light" onclick="closeModal()">Bezárás</button></div><p><b>Cég:</b> '
    +esc(x.company||'—')
    +'</p><p><b>Elérhetőség:</b> '
    +esc(x.contact)
    +'</p><p><b>Érkezett:</b> '
    +esc(x.created_at)
    +'</p><p><b>Forrás:</b> '
    +esc(x.source_code?location.origin+'/'+x.source_code:'Normál oldal')
    +'</p><hr><p style="white-space:pre-wrap">'
    +esc(x.message||'—')
    +'</p></div>';
...
async function createLink(){
  ...
  closeModal();
  await loadLinks();
  alert('Elkészült: '+location.origin+'/'+d.code);
}
...
async function apiHandler(request, env) {
  ...
  const msgMatch = url.pathname.match(
